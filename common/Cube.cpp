...
Cube::Cube() {
	...
	// Loading texture
	Texture = loadTexture("uvtemplate.DDS");
	if(Texture == 0) {	
	...
	glVertexAttribPointer(
		0,			// attribute 0. no particular reasor for 0,
		3,			// size
		GL_FLOAT,	// type
		GL_FALSE,	// normalized?
		0,			// stride
...
void Cube::draw(GLuint TextureID) {
		...
		// Set our "mytTextureSampler" sampler to use Texture Unit 0
		glUniform1i(TextureID, 0);
		glBindTexture(GL_TEXTURE_2D, Texture);

/*
		Now this code is useless since I'm using VAO to render
		// 1st attribute buffer: vertices
		glEnableVertexAttribArray(0);
		glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer);
		glVertexAttribPointer(
